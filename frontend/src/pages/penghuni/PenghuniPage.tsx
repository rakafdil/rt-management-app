...
// import PageHeader dari file komponen AppShell/Layout milikmu
...
import { useGetPenghuni, useDeletePenghuni } from "@/features/penghuni/hooks/usePenghuni";
...
export function PenghuniPage() {
  ...
  if (isLoading) return <div>Sedang memuat data dari server...</div>;
  if (isError) return <div className="text-red-500">Gagal mengambil data penghuni!</div>;
            ...
            <TableBody>
              ...
              {penghuni?.map((p) => (
                <TableRow key={p.id}>
                  <TableCell>
                    <div className="flex items-center gap-3">
                      <Avatar className="h-9 w-9">
                        {p.foto_ktp ? <AvatarImage src={p.foto_ktp} /> : null}
                        <AvatarFallback>
                          {p.nama_lengkap.split(" ").map((x) => x[0]).slice(0, 2).join("").toUpperCase()}
                        </AvatarFallback>
                      </Avatar>
                      <div>
                  ...
                  <TableCell>
                    <Badge variant={p.status_penghuni === "tetap" ? "default" : "secondary"}>
                      {p.status_penghuni}
                    </Badge>
                  </TableCell>
                  <TableCell className="font-mono text-sm">{p.nomor_telepon || "-"}</TableCell>
                  <TableCell>{p.status_menikah ? "Menikah" : "Belum menikah"}</TableCell>
